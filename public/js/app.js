...
window.cloudTeamCore = __webpack_require__(45);
...
$(function () {
	...
	function autoCancelAppointment() {
		var url = route('appointments.auto_cancel', $('#txt_appointment_id').val());
...
var cloudTeamCore = function ($, lang) {
	...
	var handleAlphanum = function handleAlphanum() {
	...
	var handleBootbox = function handleBootbox() {
		if ((typeof bootbox === 'undefined' ? 'undefined' : _typeof(bootbox)) === 'object') {
			var language = $('html').attr('lang');
	...
	var handleDatatables = function handleDatatables() {
		// noinspection JSUnresolvedVariable
		if (typeof $.fn.dataTable === 'function') {
			var language = $('html').attr('lang');
			var optionLang = {
				'sLengthMenu': 'Display <select class="m_selectpicker m-bootstrap-select--air" data-width="85px">' + '<option value="10">10</option>' + '<option value="20">20</option>' + '<option value="30">30</option>' + '<option value="40">40</option>' + '<option value="50">50</option>' +
			...
			if (language === 'vi') {
				optionLang = {
					...
					'sLengthMenu': 'Xem <select class="m_selectpicker m-bootstrap-select--air" data-width="85px">' + '<option value="10">10</option>' + '<option value="20">20</option>' + '<option value="30">30</option>' + '<option value="40">40</option>' + '<option value="50">50</option>' +
					// '<option value="-1">All</option>' +
					'</select> dòng',
			...
			// noinspection JSUnresolvedVariable
			$.fn.dataTable.Api.register('actionDelete()', function () {
				var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
				    btnDelete = _ref.btnDelete,
				    _ref$message = _ref.message,
				    message = _ref$message === undefined ? lang['Do you want to continue?'] : _ref$message,
				    _ref$title = _ref.title,
				    title = _ref$title === undefined ? lang['Delete data !!!'] : _ref$title,
				    _ref$size = _ref.size,
				...
				var checkbox = btnDelete.parents('tr').find('.m-checkbox--single > [type="checkbox"]');
				...
				btnDelete.confirmation(function (confirm) {
					if (confirm && (typeof confirm === 'undefined' ? 'undefined' : _typeof(confirm)) === 'object' && confirm.value) {
						axios.delete(url, { data: params }).then(function (result) {
							var obj = result['data'];
							flash(obj.message);
							table.reload();
			...
			$.fn.dataTable.Api.register('actionEdit()', function () {
				var _ref2 = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
				    ...
				    message = _ref2$message === undefined ? lang['Do you want to continue?'] : _ref2$message,
				    _ref2$title = _ref2.title,
				    title = _ref2$title === undefined ? lang['Edit data !!!'] : _ref2$title;

				var table = this;
				var url = btnEdit.data('url');
				var checkbox = btnEdit.parents('tr').find('.m-checkbox--single > [type="checkbox"]');
    ...
    */
	...
	var handleInput = function handleInput() {
		...
		$(document).on('click', '.m-checkbox--all > [type="checkbox"]', function () {
			var cbSingle = $('.m-checkbox--single > [type="checkbox"]');
			if (!$(this).is(':checked')) {
		...
		$(document).on('click', '.m-checkbox--single > [type="checkbox"]', function () {
			var cbSingle = $('.m-checkbox--single > [type="checkbox"]');
			var cbAll = $('.m-checkbox--all > [type="checkbox"]');
			if ($(this).is(':checked')) {
				var cbSingleChecked = $('.m-checkbox--single > [type="checkbox"]:checked');
	...
	var handleValidation = function handleValidation() {
		...
		$.validator.addMethod('pwCheck', function (value) {
			return (/^\S*(?=\S{8,})(?=\S*[a-z])(?=\S*[A-Z])(?=\S*[\d])\S*$/.test(value)
		...
		$.validator.setDefaults({
			showErrors: function showErrors() {
				...
				var msg = numberOfInvalids + lang[' field(s) are invalid'];
	...
	var handleHighcharts = function handleHighcharts() {
	...
	var handleToastr = function handleToastr() {
		if ((typeof toastr === 'undefined' ? 'undefined' : _typeof(toastr)) === 'object') {
			...
			$.fn.toast = function (options) {
				if (options.hasOwnProperty('content')) {
					...
					var _content = void 0;
					if (options.type === 'success') {
						_content = lang['Success'];
						toastr.success(_content, options.title, opts);
					} else if (options.type === 'error') {
						_content = lang['Fail'];
						toastr.error(_content, options.title, opts);
					} else if (options.type === 'warning') {
						_content = lang['Warning'];
						toastr.warning(_content, options.title, opts);
					}
				}
			};
		}
	...
	var handlePluginJquery = function handlePluginJquery() {
		/**
   ...
   */
		$.fn.confirmation = function (callback) {
			var _ref7 = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {},
			    _ref7$confirmType = _ref7.confirmType,
			    confirmType = _ref7$confirmType === undefined ? 'swal' : _ref7$confirmType,
			    _ref7$text = _ref7.text,
			    text = _ref7$text === undefined ? lang['Do you want to continue?'] : _ref7$text,
			    _ref7$title = _ref7.title,
			    title = _ref7$title === undefined ? lang['Confirm action!!!'] : _ref7$title,
			    _ref7$type = _ref7.type,
			    type = _ref7$type === undefined ? 'warning' : _ref7$type;
   ...
   */
		...
		$.fn.setModalMaxHeight = function () {
			...
			var borderWidth = $content.outerHeight() - innerHeight;
			...
			var $scrollableElem = $element.find('.m-scrollable');

			if ($scrollableElem.length !== 0 && typeof mUtil.scrollerInit === 'function' && innerHeight > 700) {
				mUtil.scrollerInit($scrollableElem[0], {
					disableForMobile: false, handleWindowResize: true, height: function height() {
   ...
   */
    ...
    */
	...
	var handleMomentJs = function handleMomentJs() {
	...
	var ajaxErrorHandler = function ajaxErrorHandler(err) {
...
var update = __webpack_require__(52)("75d14d1b", content, false, {});
...
/* harmony default export */ __webpack_exports__["default"] = ({
	...
	created: function created() {
		...
		window.events.$on('flash', function (message, level, hide) {
			_this.flash(message, level, hide);
		});

		window.events.$on('hide', function () {
			_this.hide();
		});
	},
